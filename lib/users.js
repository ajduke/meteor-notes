...
getProfileUrl = function(user) {
  return Meteor.absoluteUrl()+baseUrl+'/users/' + slugify(getUserName(user));
}
getProfileUrlById = function(id){
  return Meteor.absoluteUrl()+baseUrl+'/users/'+ id;
}
getProfileUrlBySlug = function(slug) {
  return Meteor.absoluteUrl()+baseUrl+'/users/' + slug;
}
